...
class users extends CI_Controller {
	...
	public function register() {
		...
		$this->form_validation->set_rules('password', 'Password', 'required|min_length[6]|matches[confirm_password]|trim|md5');
		$this->form_validation->set_rules('confirm_password', 'Confirm Password', 'required|min_length[4]|trim|md5');
		...
		if($this->form_validation->run())
		{
			...
			{
				$this->session->set_flashdata('dob', '<p>Please enter valid D.O.B.</p>');
				redirect('/');
			}
		}
		else 
	...
	public function login() 
	{
		...
		$this->form_validation->set_rules('password2', 'Password', 'required|min-length[6]|trim|md5');
		if($this->form_validation->run())
		{
			$user = $this->input->post();
			$this->session->set_userdata('user', $user);
